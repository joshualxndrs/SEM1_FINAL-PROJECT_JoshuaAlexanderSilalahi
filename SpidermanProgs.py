...
#DEFINING and ADJUSTING GAME and MENU BACKGROUND
#GAME BG
def draw_bg():
    screen.fill(BG)
    ...
    for n in range(5):
        ...
        screen.blit(city_img, ((n * width) - bg_scroll * 0.6, SCREEN_HEIGHT - city_img.get_height() - 300))
...
def draw_menu():
    screen.fill(BG)
    width = menu_web.get_width()
    for n in range(1):
        screen.blit(menu_web, ((n * width) - bg_scroll * 0.5, 0))
...
#---------------------------------------------------------------------------------------------------------------------------------------#
#PROGRAMS - Player (Spiderman) and Enemies (Venom)
...
class Spidey(pygame.sprite.Sprite):
    ...
    def shoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            ...
            bullet = Web(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery, self.direction)
    ...
    def venomshoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            self.shoot_cooldown = 20
            bullet = VenomWeb(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery, self.direction)
            bullet_group.add(bullet)
            #reduce ammo
            self.ammo -= 1
            shot_fx.play()
...
dead_animation = ScreenFade(2, RED, 4)
...
while run:
    ...
    if start_game == False:
        ...
        #show ammo
        draw_text('WEB: ', font, WHITE, 10, 35)
        for x in range(player.ammo):
            screen.blit(web_ammo, (90 + (x * 10), 40))
        #show grenades
        draw_text('WEB-BOMB: ', font, WHITE, 10, 60)
        ...
        #show intro
        if start_intro == True:
            if intro_animation.fade():
                start_intro = False
                intro_animation.fade_counter = 0
        ...
        if player.alive:
            ...
            #throw grenades
                ...
                grenade_thrown = True
            if player.in_air:
                player.update_action(2)#2 represents JUMP
            elif moving_left or moving_right:
                player.update_action(1)#1 represents RUN
            else:
                player.update_action(0)#0 represents IDLE state
